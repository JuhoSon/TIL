...
# # 1 5 7 8 3
# # 5 5 4 6 8
def backtracking(row_idx, visited):
    global globalMin, globalSum
    if row_idx >= N:  # 범위 벗어날 때
        if globalSum < globalMin:
            globalMin = globalSum
            return

    if globalSum > globalMin:  # prunning
        return

    for col_idx in range(N):
        if visited[col_idx] == 0:  # col_idx에 아직 방문전이라면
            visited[col_idx] = 1  # col_idx의 flag 변경 후
            globalSum += matrix[row_idx][col_idx]  # 값 더하기

            backtracking(row_idx + 1, visited)  # 업데이트 된 값으로 재귀

            visited[col_idx] = 0  # 재귀빠져나올 때 다시 원래 상태로.
            globalSum -= matrix[row_idx][col_idx]
