"""
원형 Queue
- fixed array로 Circular Queue를 구현할 때 가장 유의해야 할 점은 인덱스 컨트롤
- 나머지 연산으로 인덱스 조정을 해야 front와 rear가 가리키는 값을 정확하게 파악 가능

잘못된 포화 상태 인식
 - 선형 큐를 활용하여 enQueue 혹은 deQueue 연산을 수행 할 경우 리스트의 앞에
  데이터의 삽입을 할 수 있는 공간이 있음에도 rear = n - 1 (포화 상태)로 인식 하여
  더 이상의 삽입이 이루어지 지않는 경우가 생길 수 있음

해결 방법
1. enQueue 혹은 deQueue 연산이 이루어질 때마다 저장된 원소를 배열의 앞으로 이동
 - 원소 이동 시간만큼 Queue의 효율성이 매우 떨어진다.
2. 원형 Queue 활용
 - 1차원 배열을 사용하긴 하지만 논리적으로는 배열의 처음과 끝이 연결되어
  원형 상태의 Queue를 사용한다고 생각 (논리적 구조) -> 선형큐
"""
...
"""
# 원형 큐의 경우 front 위치의 한 칸의 공간을 두고 큐의 공백 및 포화 상태의 구분을 용이하게 함
SIZE = 5
Q = [0] * SIZE
# 초기 공백 상태
front = rear = 0
...
def dequeue():
    ...
    if is_empty():
        ...
        """
        front = (front + 1) % len(Q)
        # 실제 값을 큐에서 삭제하지 않아도 돌고 돌아 다른 값으로 덮어 써지기 때문에 상관없다.
        # 즉, 반환만 해도 문제없다.
        return Q[front]
...
Q => [4, 5, 2, 3]
"""
print(front, rear, Q) # 0 0 [5, 1, 2, 3, 4]